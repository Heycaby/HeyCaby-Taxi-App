...
final driverRuntimeSnapshotProvider =
    FutureProvider<DriverRuntimeSnapshot>((ref) async {
  return ref.read(driverRuntimeServiceProvider).fetchRuntime();
});

final driverRemoteConfigProvider =
    FutureProvider<DriverRemoteConfig>((ref) async {
  final runtime = await ref.watch(driverRuntimeSnapshotProvider.future);
  return runtime.config;
});

final driverReadinessProvider =
    FutureProvider<DriverReadinessState?>((ref) async {
  final runtime = await ref.watch(driverRuntimeSnapshotProvider.future);
