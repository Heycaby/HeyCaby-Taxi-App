...
import { anonKey, createClient, serviceRoleKey, supabaseUrl, WEBHOOK_SECRET } from './config.ts'
...
export async function deliverNotification(
  ...
  if (n.user_id === '' && n.data?.ride_request_id) {
    ...
    if (!driverId) return ok()
    n = { ...n, user_id: driverId }
  }
  if (!n.user_id) return ok()

  if (n.category === 'preride_request' && n.data?.ride_request_id) {
    const dup = await recentPrerideNotification(supabase, n.data.ride_request_id as string)
    if (dup) return ok()
  }
  ...
  await supabase.from('agent_logs').insert({
    agent: 'driver_agent',
    event_type: typeof payloadForLog === 'object' && payloadForLog !== null && 'table' in (payloadForLog as object)
      ? `${(payloadForLog as WebhookPayload).table}_${(payloadForLog as WebhookPayload).type}`
...
export async function authorizeManualPreride(req: Request, rideRequestId: string): Promise<boolean> {
  if (WEBHOOK_SECRET) {
  ...
  const { data: ride } = await sr.from('ride_requests').select('driver_id').eq('id', rideRequestId).maybeSingle()
  const driverFk = ride?.driver_id as string | undefined
  if (!driverFk) return false
  const { data: drv } = await sr.from('drivers').select('user_id').eq('id', driverFk).maybeSingle()
