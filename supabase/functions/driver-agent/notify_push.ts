...
export async function getDriverIdForRideRequest(supabase: SupabaseClient, rideRequestId: string): Promise<string | null> {
  const { data } = await supabase.from('ride_requests').select('driver_id').eq('id', rideRequestId).single()
