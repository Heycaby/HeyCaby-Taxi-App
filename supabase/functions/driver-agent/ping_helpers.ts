...
function vehicleLabel(driver: Record<string, unknown> | null | undefined): string {
  if (!driver) return ''
  const make = (driver.rdw_merk ?? driver.vehicle_make ?? '') as string
  const model = (driver.rdw_handelsbenaming ?? driver.vehicle_model ?? '') as string
...
): { title: string; body: string; priority: AgentNotificationRow['priority']; channelId: string } {
  ...
  switch (kind === 'nearby' ? 'on_my_way' : kind) {
    case 'outside':
      return {
        ...
        body: `Je chauffeur wacht buiten.${vehicleBlock}\n\nKom alsjeblieft naar buiten.`,
        priority: 'critical',
    ...
    case 'cant_find_rider':
      return {
        ...
        body: `Je chauffeur kan je niet vinden. Stuur een chatbericht of kom naar buiten.${vehicleBlock}`,
        priority: 'critical',
        channelId: 'heycaby_ping_urgent',
      }
    case 'thanks':
    ...
    default:
      return {
        ...
        body: `Je chauffeur is onderweg naar het ophaalpunt.${etaBlock}${vehicleBlock}`,
        priority: 'high',
        channelId: 'heycaby_ping_standard',
      }
  }
