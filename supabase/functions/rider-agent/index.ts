...
import { serviceRoleKey, supabaseUrl, WEBHOOK_SECRET } from "./config.ts";
...
Deno.serve(async (req) => {
  try {
    ...
    if (webhook?.table === "notifications" && webhook?.type === "INSERT") {
      ...
      if (
        ...
        if (notification) {
          ...
          const liveActivityPushed = await deliverLiveActivityUpdate(
            supabase,
            riderNotification,
          );
          ...
          return json({
            ...
            live_activity_pushed: liveActivityPushed,
          });
        }
      }
      return ok();
    ...
    if (manual?.notification_id) {
      ...
      if (
        !notification ||
        !["rider_agent", "ride_lifecycle"].includes(notification.agent)
      ) {
