...
function contentFor(
  ...
  const terminal = ['cancelled', 'canceled', 'rejected', 'declined', 'missed', 'expired'].includes(status)
  const paid = payment === 'paid' || category === 'payment_confirmed'
  ...
  const onTrip = status === 'in_progress' || category.includes('trip_started') || category.includes('ride_started')
  const onWay = status === 'driver_en_route' || category.includes('on_my_way') || category.includes('en_route')
  ...
  if (paid) {
    phase = 'completed'; progressPercent = 100; heroMetric = '✓'; compactTrailing = 'Done'
    nextAction = 'Thank you for riding with HeyCaby.'
  } else if (completed) {
    phase = 'payment'; progressPercent = 96; heroMetric = 'Pay'; compactTrailing = 'Pay'
    nextAction = 'Confirm payment with your driver.'
  } else if (onTrip) {
    phase = 'on_trip'; progressPercent = 78
    const minutes = Math.max(1, Number(ride.estimated_duration_min ?? 0))
    eta = minutes > 0 ? `${minutes} min` : ''
    heroMetric = eta; compactTrailing = eta || 'Trip'
    nextAction = "Relax — we'll keep you updated."
  } else if (arrived) {
    phase = 'outside_free'; progressPercent = 62; waitPhase = 'free'
    ...
    if (Number.isFinite(arrivedMs)) {
      ...
      const remaining = Math.max(0, graceEndsAtEpoch - Math.floor(Date.now() / 1000))
      graceRemaining = `${Math.floor(remaining / 60)}:${String(remaining % 60).padStart(2, '0')}`
      if (remaining === 0 && !ride.waiting_fee_waived) {
        phase = 'outside_paid'; waitPhase = 'paid'
        const cents = Math.max(0, Number(ride.waiting_fee_cents ?? 0))
        waitFee = cents > 0 ? `€${(cents / 100).toFixed(2)} added` : 'Waiting fee active'
      }
    }
    ...
    phase = 'nearby'; progressPercent = 52; heroMetric = 'Nearby'; compactTrailing = 'Near'
    nextAction = 'Please head downstairs to avoid waiting fees.'
  } else if (onWay) {
    phase = 'on_the_way'; progressPercent = 45; compactTrailing = 'En route'
...
export async function deliverLiveActivityUpdate(
  ...
  notification: { category: string; title: string; body: string; data: Record<string, unknown> },
): Promise<number> {
  ...
  const payload = contentFor(ride as RideRow, notification.category, notification.title, notification.body)
  ...
  for (const activity of activities) {
    ...
    const sent = await sendFcmLiveActivityUpdate({
      ...
      dismissalDate: payload.event === 'end' ? Math.floor(Date.now() / 1000) + 60 : undefined,
    })
