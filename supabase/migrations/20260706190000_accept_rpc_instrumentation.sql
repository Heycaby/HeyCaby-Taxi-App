-- P0: Instrument accept RPC — structured reason/details + audit on every rejection.

CREATE OR REPLACE FUNCTION public.fn_accept_invite_diagnostic(
  p_ride_request_id uuid,
  p_driver_id uuid,
  p_accept_grace_seconds int DEFAULT 30
)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'invite_expired', v_invite.expires_at <= now(),
    'invite_in_grace',
      v_invite.expires_at > now() - make_interval(secs => p_accept_grace_seconds),
    'seconds_until_expiry',
      EXTRACT(EPOCH FROM (v_invite.expires_at - now()))::int,
    'batch_no', v_invite.batch_no
  );
END;
$$;
...
COMMENT ON FUNCTION public.fn_driver_accept_ride_invite(uuid) IS
  'Atomic accept with structured reject reasons (dispatch.accept_rejected audit on failure).';
