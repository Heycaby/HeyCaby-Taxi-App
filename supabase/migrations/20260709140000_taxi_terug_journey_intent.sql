-- Taxi Terug Phase 4: Driver Journey Intent
-- Extends Return Mode to support planned direction trips, not just post-ride returns.
-- A driver can set: intent_type, destination, departure_time, pickup_radius, destination_radius.
-- Matching logic now includes timing fit for planned trips.

-- ─── New columns on drivers ────────────────────────────────────────────────
ALTER TABLE public.drivers
  ADD COLUMN IF NOT EXISTS return_mode_intent_type text
    NOT NULL DEFAULT 'post_ride_return'
    CHECK (return_mode_intent_type IN (
      'home', 'airport', 'city', 'custom',
      'post_ride_return', 'planned_direction'
    )),
  ADD COLUMN IF NOT EXISTS return_mode_departure_time timestamptz,
  ADD COLUMN IF NOT EXISTS return_mode_destination_radius_km numeric
    NOT NULL DEFAULT 5;

COMMENT ON COLUMN public.drivers.return_mode_intent_type IS
  'Type of journey intent: home, airport, city, custom, post_ride_return, planned_direction.';
COMMENT ON COLUMN public.drivers.return_mode_departure_time IS
  'When the driver plans to leave. NULL means immediate (now).';
COMMENT ON COLUMN public.drivers.return_mode_destination_radius_km IS
  'How far (km) the rider destination can be from the driver planned destination.';
...
SET value = (
  ...
  || jsonb_build_object(
    ...
    'max_departure_delay_hours', 12
  )
)::text
WHERE key = 'terugtaxi_config';

-- ─── Updated fn_taxi_terug_config ──────────────────────────────────────────
...
-- ─── Updated fn_driver_return_mode_status (adds journey intent fields) ─────
...
BEGIN
  SELECT
    ...
    d.return_mode_intent_type,
    d.return_mode_departure_time,
    d.return_mode_destination_radius_km,
  ...
  RETURN jsonb_build_object(
    ...
    'intent_type', coalesce(v_driver.return_mode_intent_type, 'post_ride_return'),
    'departure_time', v_driver.return_mode_departure_time,
    'destination_radius_km', coalesce(v_driver.return_mode_destination_radius_km, 5)
  );
END;
$$;

-- ─── Updated fn_driver_return_mode_activate (adds journey intent params) ───
...
BEGIN
  ...
  -- Validate departure_time (not too far in the future)
...
-- ─── Updated fn_rider_taxi_terug_candidates (adds departure time + timing) ─
...
BEGIN
  ...
  metrics AS (
    SELECT
      ...
      -- Timing fit: for planned_direction with departure_time, check if rider pickup fits window
      CASE
...
-- ─── Updated fn_terugtaxi_qualify (adds timing check for planned trips) ────
...
BEGIN
  ...
  -- Timing check for planned_direction trips
  IF v_driver.return_mode_departure_time IS NOT NULL THEN
    IF now() + make_interval(mins => round(v_pickup_distance_km * COALESCE((v_cfg->>'eta_minutes_per_km')::numeric, 2.2))::int)
       > v_driver.return_mode_departure_time + make_interval(mins => v_departure_window_min) THEN
      v_timing_ok := false;
    END IF;
  END IF;
  ...
  ELSIF NOT v_timing_ok THEN
    v_reason := 'timing_mismatch';
  ELSE
    v_allowed := true;
    v_reason := 'qualified';
  END IF;
...
REVOKE ALL ON FUNCTION public.fn_driver_return_mode_activate(text, uuid, double precision, double precision, numeric, numeric, text, timestamptz, numeric) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.fn_driver_return_mode_activate(text, uuid, double precision, double precision, numeric, numeric, text, timestamptz, numeric) TO authenticated, service_role;
