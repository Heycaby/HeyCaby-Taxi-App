-- Consolidate duplicate push registration RPC overloads and retain whether an
-- iOS APNs token came from a debug/sandbox or release/production entitlement.
...
drop function if exists public.fn_register_push_device(text, text, text, uuid);
drop function if exists public.fn_register_push_device(text, text, text, uuid, text);

create function public.fn_register_push_device(
  p_fcm_token text,
  p_platform text,
  p_app_role text,
...
begin
  ...
  if p_platform = 'ios' and p_apns_token is not null
    ...
    v_apns_environment := coalesce(p_apns_environment, 'production');
  end if;
...
comment on function public.fn_register_push_device(text, text, text, uuid, text, text) is
  'Canonical FCM/APNs device registration. APNs entitlement environment is server persisted for correct provider routing.';
